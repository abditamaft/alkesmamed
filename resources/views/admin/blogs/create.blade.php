...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-black text-gray-800">Tulis Artikel Baru</h1>
</div>
    ...
    <div class="lg:col-span-2 space-y-6">
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            <div class="mb-6">
                ...
                <input type="text" name="title" required placeholder="Contoh: Manfaat Menggunakan Masker Medis..." class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:border-blue-500 font-bold text-gray-800">
            </div>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 sticky top-6">
            ...
            <div class="mb-6">
                ...
                <div class="border-2 border-dashed border-gray-300 rounded-xl p-4 text-center hover:bg-gray-50 transition relative" x-data="{ imageUrl: null }">
                    <input type="file" name="image" required accept="image/*" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" @change="imageUrl = URL.createObjectURL($event.target.files[0])">
                    
                    <div x-show="!imageUrl" class="py-6">
                        <i class="fa-solid fa-cloud-arrow-up text-3xl text-gray-300 mb-2"></i>
                        <p class="text-xs font-bold text-gray-500">Klik atau Drag foto ke sini</p>
                        <p class="text-[10px] text-gray-400 mt-1">Format: JPG, PNG, WEBP (Max 2MB)</p>
                    </div>
                    <img x-show="imageUrl" :src="imageUrl" class="w-full h-40 object-cover rounded-lg shadow-sm" style="display: none;">
                </div>
            </div>

            <div class="mb-6 bg-blue-50 p-4 rounded-xl border border-blue-100 flex items-center justify-between" x-data="{ isPublished: true }">
                <span class="text-sm font-bold text-blue-800">Publikasikan Langsung?</span>
